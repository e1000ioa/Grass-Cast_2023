...
J <- ggplot(JFM, aes(x = JFM$x, y = JFM$y, label=JFM$year)) +
  ...
  annotate(
    ...
    x = -1.10,
    y = 35,
    label = paste("p value =", signif(summary(model_JFM)$coefficients[2,4], digits = 2)),
    color = "#595959",
    size = 5
  ) + #Annotate p value
...
M <- ggplot(MAM, aes(x = x, y = y,label=MAM$year)) +
  ...
  annotate(
    ...
    x = -1.10,
    y = 35,
    label = paste("p value =", signif(summary(model_MAM)$coefficients[2,4], digits = 2)),
    color = "#595959",
    size = 5
  ) + #Annotate R2
