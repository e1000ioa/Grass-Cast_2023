...
map_annp <- function(date,season,measure,unit,colname) {
  ...
  a <- ggplot() +
    # Add the raster and modify the scale
    geom_raster(data = cr_raster, aes(x = x, y = y, fill = cut(layer, breaks = c(0.36, 0.8, 1.2, 3), labels = c("0.36-0.8", "0.8-1.2", "1.2-2.96"), include.lowest = TRUE))) +
    scale_fill_manual(
      name = "Ratio",
      values = c("#003399", "#009900", "#ff3333"),
      drop = FALSE
    ) +
    ...
    geom_sf(data = nat, color=alpha("#000000",0.6), linewidth=0.5,linetype = "dotted", fill = NA) +
...
map_annp(Forecast_List[1],"SUMMER FORECAST","ANPP","(%)","pptRatioSummerWinter")
