...
ggplot(df, aes(x = x, y = y)) +
  ...
  annotate("text", x = median(x), y = median(y), label = paste0("R2 = ", round(rsq, 2)),
           ...
           hjust = 0)+
  annotate("text", x = median(x), y = (max(y)/2), label = paste0("P value = ", format.pval(pval))
           ,color = c, size = 8, fontface = "bold", alpha = 1, 
           hjust = 0)+
  annotate("text", x = median(x), y = (max(y)/4), label = paste0("Slope = ", format(round(slope, 2), nsmall = 2)),
           color = c, size = 8, fontface = "bold", alpha = 1, 
           hjust = 0) +
...
#SUMMER
#MAM
ENSO(ENSOvsSU_1950_2020,4,8,"darkgreen","Summer")

#JJA
ENSO(ENSOvsSU_1950_2020,5,8,"#cb4154","Summer")
